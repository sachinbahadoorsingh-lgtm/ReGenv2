...
export default async function handler(req) {
  ...
  if (!userName || !password || !database) return new Response('Missing userName/password/database', { status: 400 });
  ...
  if (!sessionId) return new Response('Authenticate returned no sessionId', { status: 400 });

  // Return baseUrl + EXACT credentials from server (plus username for convenience)
  return json({
    baseUrl,
    database: db,
    sessionId,
    userName,
    credentials: result.credentials   // <— important
  });
}
